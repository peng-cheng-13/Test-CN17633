...
public:
	...
	AE_NO_TSAN explicit ReaderWriterQueue(size_t maxSize = 15, int shmid = 0)
#ifndef NDEBUG
		: enqueuing(false)
		,dequeuing(false)
#endif
	{
		...
		if (largestBlockSize > MAX_BLOCK_SIZE * 2) {
			...
			for (size_t i = 0; i != initialBlockCount; ++i) {
				auto block = make_block(largestBlockSize, shmid);
				if (block == nullptr) {
			...
			firstBlock = make_block(largestBlockSize, shmid);
			if (firstBlock == nullptr) {
#ifdef MOODYCAMEL_EXCEPTIONS_ENABLED
				throw std::bad_alloc();
#else
...
private:
	...
	static Block* make_block(size_t capacity, int shmid) AE_NO_TSAN
        {
          auto newBlockRaw = static_cast<char*>(shmat(shmid,NULL,0));
