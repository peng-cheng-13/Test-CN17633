...
std::string ufsPath = "/home/condor/test/H5SetAttribute/ExtractMetadata";
...
int main(int argc, char *argv[]) {
    ...
    for(int i = rank; i < buf.size(); i += size) {
      ...
      hid_t file = H5Fopen(tmpfile, H5F_ACC_RDWR, H5P_DEFAULT);
    ...
    if (rank == 0) {
      ...
      float readlist_time = (t4.tv_sec - t3.tv_sec)*1000 + (t4.tv_usec - t3.tv_usec)/1000;
      float extract_time = (t6.tv_sec - t5.tv_sec)*1000 + (t6.tv_usec - t5.tv_usec)/1000;
      printf("init_time is %f ms\n", init_time);
      printf("readlist_time is %f ms\n", readlist_time);
      printf("extract_time is %f ms\n", extract_time);
    }
    return 0;
}
