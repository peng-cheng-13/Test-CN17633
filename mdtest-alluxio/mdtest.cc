...
#else
	client->createDirectory(curr_item);
        /*if (mkdir(curr_item, DIRMODE) == -1) {
          FAIL("unable to create directory");
        }*/
#endif
...
#else
	client->deletePath(curr_item, true);
#endif
      }
    /*
     * !dirs
 ...
 */
          ...
          jFileOutStream fileOutStream = client->createFile(curr_item, NULL);
          fileOutStream->close();
          /*if ((fd = open(curr_item, O_RDWR)) == -1) {
            FAIL("unable to open file");
          }*/
...
#else
	  client->deletePath(curr_item, false);
#endif
        }
      }
    }
...
#else
      if (fin->read(read_buffer, read_bytes) != read_bytes) {
        FAIL("unable to read file");
      }
...
#else
	client->createDirectory(temp_path);
#endif
      }
...
#else
	client->deletePath(temp_path, true);
#endif
      }
...
int main(int argc, char **argv) {
...
#else
    if ((rank < path_count) && access(testdirpath, F_OK) != 0) {
      client->createDirectory(testdirpath);
    }
#endif
    ...
    for (i = first; i <= last && i <= size; i += stride) {
        //range.last = i - 1;
        range[0][1] = i - 1;
        MPI_Group_range_incl(worldgroup, 1, range, &testgroup);
        ...
        for (j = 0; j < iterations; j++) {
	       ...
	       client->createDirectory(testdir);
            ...
            if (remove_only) {
                ...
                if ((rank < path_count) && access(testdir, F_OK) == 0) {
                //if (( rank == 0 ) && access(testdir, F_OK) == 0) {
                client->deletePath(testdir, true);
                }
#endif
            } else {
